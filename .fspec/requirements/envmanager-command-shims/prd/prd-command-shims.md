...
- **当前阶段**：问题确认
- **当前状态**：待确认
- **最后完成步骤**：已完成背景、目标、推荐方案、功能边界和验收骨架
- **当前中断点**：首期能力模型尚未确认
- **下一步动作**：确认“通用 Command Shim”或“仅 Node 脚本命令”
...
- **输出级别**：PRD 骨架 + 待确认项
- **选择原因**：背景和目标清晰，但首期能力模型、产品版本和高级字段范围尚未确认
- **完成条件**：P0 范围问题确认，并完成验收标准复核
...
| 编号 | 优先级 | 问题 | 推荐答案 | 状态 |
|------|--------|------|----------|------|
| Q1 | P0 | 首期是否做通用“可执行文件 + 固定参数”Shim？ | 是；覆盖 Node、Python 和原生 CLI | 待确认 |
| Q2 | P1 | 是否仅支持 User scope？ | 是；不引入 UAC 和全局冲突 | 待确认 |
| Q3 | P1 | 是否允许在应用内试运行？ | 首期不允许，只验证配置和生成物 | 待确认 |
| Q4 | P1 | 目标产品版本是什么？ | 下一个 minor 版本 | 待确认 |
| Q5 | P2 | 是否支持工作目录和额外环境变量？ | 首期不支持 | 待确认 |
...
- 需要从 PowerShell、CMD、IDE Terminal 或子进程稳定调用内部 CLI 的用户。
...
- 待确认：首期不支持系统级 Shim、工作目录、按项目生效和额外环境变量注入。
...
| Executable | 实际启动的可执行文件绝对路径 | `C:\Users\mulia\scoop\apps\nodejs-lts\current\node.exe` |
...
| 字段 | 说明 |
|------|------|
| Managed shim path | EnvManager 生成的 `.cmd` 文件路径 |
...
7. 用户确认保存。
8. 系统原子生成受管理 Shim，并确保 managed bin directory 位于 User `PATH`。
9. 页面显示成功状态，并提示新终端可直接调用 `sharedev`。

**后置条件**：新启动的 PowerShell、CMD 和 IDE Terminal 能解析 `sharedev`。

**异常流程**：

- 命令名不合法：阻止保存并定位字段。
- executable 不存在：阻止保存。
- fixed argument 看起来是文件路径但不存在：默认阻止保存；是否允许强制保存待确认。
...
- 删除最后一个 Shim 后，是否自动从 User `PATH` 移除 managed directory 待确认；推荐保留，避免反复修改 `PATH`。
...
- UI 可提供粘贴完整命令并解析为结构化字段的辅助入口，但解析结果必须让用户确认；首期是否提供待确认。
...
2. 在新 PowerShell 中执行 `sharedev --help` 时，实际进程使用配置的 Node.js，而非 `PATH` 中的默认 Node.js。
...
2. managed directory 中存在外部同名 `.cmd` / `.exe` 时，保存被阻止。
...
| 维度 | 覆盖项 |
|------|--------|
| Shell | Windows PowerShell 5.1、PowerShell 7、CMD、IDE Terminal |
...
```json
{
  ...
  "executable": "C:\\Users\\mulia\\scoop\\apps\\nodejs-lts\\current\\node.exe",
  "fixedArguments": [
    "D:\\work\\work\\git\\fe-paas\\web-ide\\packages\\share-dev-cli\\dist\\sharedev.js"
  ]
}
